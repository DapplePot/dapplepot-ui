  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">All client tenants and their admin accounts.</p>
      </div>
      ...
      {!isLoading && !isError && (
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
          <table className="w-full text-left">
            ...
            <tbody>
              {filtered.map((tenant) => (
                <tr key={tenant.tenantId} className="border-b border-slate-100 last:border-0 dark:border-slate-800">
                  <td className="px-4 py-3">
                    <div className="text-sm font-medium text-slate-900 dark:text-slate-100">{tenant.name}</div>
                    <div className="font-mono text-xs text-slate-400 dark:text-slate-500">{tenant.tenantId}</div>
                  </td>
                  <td className="px-4 py-3">
                    {tenant.adminUser ? (
                      <>
                        <div className="text-sm text-slate-900 dark:text-slate-100">{tenant.adminUser.name}</div>
                        <div className="text-xs text-slate-500 dark:text-slate-400">{tenant.adminUser.email}</div>
                      </>
                    ) : (
                      <span className="text-xs text-slate-400 italic dark:text-slate-500">No admin assigned</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm text-slate-700 dark:text-slate-300">{tenant.userCount}</span>
                  </td>
                  <td className="px-4 py-3">
