...
function CopyableId({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 dark:border-slate-700 dark:bg-slate-800">
      <div>
        <p className="text-[10px] font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500">{label}</p>
        <p className="mt-0.5 font-mono text-xs text-slate-700 dark:text-slate-300">{value}</p>
      </div>
      <button
        onClick={handleCopy}
        title={`Copy ${label}`}
        className="ml-4 flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2 py-1 text-[10px] text-slate-500 shadow-sm transition-colors hover:border-violet-300 hover:text-violet-600 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-400 dark:hover:border-violet-500 dark:hover:text-violet-400"
      >
        {copied
...
function PreferencesSection() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-200">Appearance</h3>
        <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">Choose how DapplePot looks to you.</p>
          ...
          {THEME_OPTIONS.map(({ value, label, icon: Icon, desc }) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[11px] text-center text-slate-400 dark:text-slate-500">{desc}</span>
              </button>
            )
          })}
        </div>
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-900">
      <div className="mb-3 flex items-center gap-2">
        <div className="flex h-7 w-7 items-center justify-center rounded-md border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
            ...
            {visibleNav.map(item => {
              ...
              return (
                <li key={item.id}>
                  ...
                  >
                    <Icon className={`h-4 w-4 shrink-0 ${isActive ? 'text-violet-500 dark:text-violet-400' : 'text-slate-400 dark:text-slate-500'}`} />
                    {item.label}
                  </button>
                </li>
              )
