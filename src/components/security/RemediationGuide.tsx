      ...
      {cards.map((card, i) => (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-slate-900 dark:text-zinc-100">{card.title}</p>
              <p className="mt-0.5 font-mono text-xs text-slate-500 dark:text-zinc-400">
                {card.owaspSignalId}
              </p>
            </div>
            <span className="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500 dark:bg-zinc-800 dark:text-zinc-400">
