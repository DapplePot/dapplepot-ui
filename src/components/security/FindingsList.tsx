        ...
        <div key={f.findingId}>
          ...
          >
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{f.checkScore}</span>
              {f.confidence !== undefined && (
                <span className="text-xs text-slate-400 dark:text-slate-500">×{f.confidence.toFixed(1)}</span>
              )}
            </div>
          ...
          {expandedId === f.findingId && (
            <div className="border-t border-slate-50 px-4 py-3 bg-slate-50 space-y-3 dark:border-slate-800 dark:bg-slate-800/50">
              {f.detail && (
              ...
              <div className="flex items-center gap-3 text-xs text-slate-400 dark:text-slate-500">
                ...
                {f.confidenceTier && (
                  <>
                    <span>·</span>
                    <span>confidence: <span className="font-medium text-slate-500 dark:text-slate-400">{f.confidenceTier}</span>
