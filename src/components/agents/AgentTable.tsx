...
export function AgentTable() {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading && (
        <p className="text-sm text-slate-500 dark:text-slate-400">Loading agents…</p>
      )}
      ...
      {!isLoading && !isError && (
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
          <table className="w-full text-left">
            ...
            <tbody>
              {filtered.map((agent) => (
                <tr key={agent.agentId} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800">
                  <td className="p-0">
                    <Link
                      to="/agents/$agentId"
                      params={{ agentId: agent.agentId }}
                      className="flex items-center px-4 py-3 text-sm font-medium text-slate-900 dark:text-slate-100"
                    >
                      {agent.name}
                    </Link>
                  </td>
                  ...
                  <td className="p-0">
                    ...
                    >
                      {agent.latestVersion
                        ? <span className="rounded-full bg-slate-100 px-2 py-0.5 font-mono text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-300">{agent.latestVersion}</span>
                  ...
                  <td className="p-0">
                    <Link
                      to="/agents/$agentId"
                      params={{ agentId: agent.agentId }}
                      className="flex items-center px-4 py-3 text-sm text-slate-500 dark:text-slate-400"
                    >
