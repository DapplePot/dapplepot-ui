    ...
    <div className="divide-y divide-slate-100 dark:divide-zinc-800">
      {rows.map(({ label, value }) => (
        <div key={label} className="flex gap-3 py-2">
          <span className="w-32 shrink-0 text-xs font-medium text-slate-500 dark:text-zinc-400">{label}</span>
          <span className="min-w-0 break-all font-mono text-xs text-slate-800 dark:text-zinc-200">
            {value ?? '—'}
          </span>
        </div>
      ))}
      ...
      {session.exitReason && (
        <div className="flex gap-3 py-2">
          <span className="w-32 shrink-0 text-xs font-medium text-slate-500 dark:text-zinc-400">Exit reason</span>
