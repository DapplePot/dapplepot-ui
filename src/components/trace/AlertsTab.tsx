...
export function AlertsTab({ alerts }: AlertsTabProps) {
  ...
  return (
    <div className="space-y-3">
      {alerts.map((alert) => (
        <div key={alert.alertId} className="rounded-lg border border-slate-200 p-3">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-0.5 text-xs text-slate-500">{alert.ruleName} · {formatAgo(alert.triggeredAt)}</p>
            </div>
            <Badge
              variant={
