        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              spellCheck={false}
              placeholder={'{\n  "path": {},\n  "encoding": {}\n}'}
              className="w-full resize-none rounded border border-slate-300 bg-slate-50 px-3 py-2 font-mono text-xs text-slate-900 placeholder-slate-400 outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder-zinc-500"
            />
            <p className="mt-1 text-xs text-slate-500 dark:text-zinc-400">
              JSON object — one key per declared parameter. TME-01a fires when tool_start carries undeclared keys.
            </p>
            {schemaErr && (
              <p className="mt-1 rounded bg-red-50 px-3 py-2 text-xs text-red-600 dark:bg-red-900/20 dark:text-red-400">{schemaErr}</p>
            )}
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded border border-slate-200 bg-white p-6 shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              spellCheck={false}
              placeholder={'{\n  "path": {},\n  "encoding": {}\n}'}
              className="w-full resize-none rounded border border-slate-300 bg-slate-50 px-3 py-2 font-mono text-xs text-slate-900 outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"
            />
            <p className="mt-1 text-xs text-slate-500 dark:text-zinc-400">TME-01a fires when tool_start carries undeclared keys.</p>
            {parseError && (
              <p className="mt-1 rounded bg-red-50 px-3 py-2 text-xs text-red-600 dark:bg-red-900/20 dark:text-red-400">{parseError}</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button type="button" onClick={onClose} className="rounded border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:border-zinc-600 dark:text-zinc-300 dark:hover:bg-zinc-800">Cancel</button>
          <button onClick={handleSave} disabled={updateTool.isPending} className="rounded bg-violet-600 px-4 py-2 text-sm font-medium text-white hover:bg-violet-500 disabled:opacity-50">
