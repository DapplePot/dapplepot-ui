...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        'relative flex flex-col border-r border-slate-200 bg-white transition-all duration-200 dark:border-slate-700 dark:bg-slate-900',
        collapsed ? 'w-14' : 'w-56'
      )}
    >
      {/* Logo */}
      <div className="flex h-14 items-center border-b border-slate-200 px-3 dark:border-slate-700">
        ...
        {!collapsed && (
          <span className="ml-2.5 truncate text-sm font-semibold text-slate-900 dark:text-slate-100">
      ...
      {/* User + logout */}
      <div className="border-t border-slate-200 px-2 py-2 space-y-1 dark:border-slate-700">
        {!collapsed && user && (
        ...
        <button
          ...
          className="flex w-full items-center gap-3 rounded-md px-2 py-2 text-sm text-slate-500 transition-colors hover:bg-slate-50 hover:text-red-600 disabled:opacity-50 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-red-400"
          aria-label="Sign out"
        >
